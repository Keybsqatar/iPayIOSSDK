...
private func loadSVG() {
    ...
    URLSession.shared.dataTask(with: url) { data, _, _ in
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            // Set a fixed size to avoid PPI calculation issues
            let targetSize = CGSize(width: 30, height: 30)
            svgImage.size = targetSize
            svgImage.scaleToFit(inside: targetSize)
            
            // Try to get the UIImage directly
            ...
            // Fallback: Use manual rendering
            DispatchQueue.main.async {
                let imageView = SVGKImageView(svgkImage: svgImage)
                imageView?.bounds = CGRect(origin: .zero, size: targetSize)
                imageView?.setNeedsLayout()
                imageView?.layoutIfNeeded()
                
                // Get UIImage from the SVGKImage, not from imageView.image
                if let renderedUIImage = imageView?.image?.uiImage {
                    SVGImageCache.shared.setImage(renderedUIImage, for: self.url)
