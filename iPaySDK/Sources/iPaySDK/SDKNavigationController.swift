...
/// UINavigationController that shows a persistent "Done" bar above the keyboard.
/// Works for the root and any SwiftUI-pushed inner screens.
public final class SDKNavigationController: UINavigationController {

    private let accessory = KeyboardAccessoryBar()
    private var accessoryBottom: NSLayoutConstraint!

    public override func viewDidLoad() {
        super.viewDidLoad()

        // Add the accessory once to the nav controller's view
        accessory.translatesAutoresizingMaskIntoConstraints = false
        view.addSubview(accessory)

        accessoryBottom = accessory.bottomAnchor.constraint(
        ...
        accessory.onDone = { [weak self] in
            // Dismiss any current first responder inside the whole stack
            self?.view.endEditing(true)
            UIApplication.shared.sendAction(#selector(UIResponder.resignFirstResponder),
                                            to: nil, from: nil, for: nil)
        }

        // Observe keyboard for all children
    ...
    @objc private func kbChange(_ n: Notification) {
        ...
        accessoryBottom.constant = -(overlap - inset)   // ⬅️ adjust for safe-area bottom
...
// Simple blur bar with trailing "Done" button
